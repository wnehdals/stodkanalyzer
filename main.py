...
def main():
    ...
    while True:
        ...
        if command == "0":
            ...
            news = get_ticker_news(symbol.upper(), ["JP모건", "모건스탠리", "뱅크오브아메리카", "골드만삭스"])
            ...
            draw_graph(symbol.upper(), [])
        # filter_bank_news()


def draw_graph(symbol: str, bankName: [str] = []):
    ...
    """
    news = filter_opinion_news(symbol, bankName)
    if not news or len(news) == 0:
        print(f"{symbol}에 대한 뉴스 데이터가 없습니다.")
        return
    ...
    for row in news:
        title = row.get("title", "")
        target_opinions.append(
            Opinion(
                symbol= symbol,
                opinion=thinking_opinion(title),
                opinion_date=row.get("created_at", ""),
                bank= get_bank_from_title(title).name,
                news_id=row.get("id", ""))
            )
    ...
    try:
        ...
        # 주가 데이터 범위를 의견 날짜보다 30일 앞뒤로 확장
        start_dt_extended = start_dt - timedelta(days=30)
        end_dt_extended = end_dt + timedelta(days=30)
        
        start_date_formatted = start_dt_extended.strftime('%Y-%m-%d')
        end_date_formatted = end_dt_extended.strftime('%Y-%m-%d')
        
        print(f"주가 데이터 요청 범위: {start_date_formatted} ~ {end_date_formatted}")
        print("NYSE 종목 목록을 가져오는 중...")
        nasdaq_df = fdr.StockListing('NYSE')
        
        df = fdr.DataReader(symbol, start_date_formatted, end_date_formatted)

        if df.empty:
            print(f"{symbol}의 주식 데이터를 가져올 수 없습니다.")
            return
        ...
        # 4. 주가 그래프 그리기
        ax.plot(df.index, df['Close'], label='Close Price', color='gray', linewidth=2)

        # 5. 의견이 있는 날짜에 점으로 표시
        opinion_colors = {'상향': 'red', '하향': 'royalblue', '중립': 'orange'}
        ...
        for opinion in target_opinions:
            ...
            try:
                ...
                if opinion_date in df.index:
                    ...
                    actual_date = opinion_date
                else:
                    # 가장 가까운 날짜의 주가 데이터 찾기
                    try:
                        # 의견 날짜보다 이전 날짜들 중 가장 가까운 것 찾기
                        available_dates = df.index[df.index <= opinion_date]
                        if len(available_dates) > 0:
                            actual_date = available_dates[-1]  # 가장 최근 날짜
                            close_price = df.loc[actual_date, 'Close']
                            print(f"의견 날짜 {opinion_date_str}에 해당하는 주가 데이터가 없어서 {actual_date.strftime('%Y-%m-%d')}의 주가를 사용합니다.")
                        else:
                            # 의견 날짜보다 이후 날짜들 중 가장 가까운 것 찾기
                            available_dates = df.index[df.index >= opinion_date]
                            if len(available_dates) > 0:
                                actual_date = available_dates[0]  # 가장 이른 날짜
                                close_price = df.loc[actual_date, 'Close']
                                print(f"의견 날짜 {opinion_date_str}에 해당하는 주가 데이터가 없어서 {actual_date.strftime('%Y-%m-%d')}의 주가를 사용합니다.")
                            else:
                                print(f"의견 날짜 {opinion_date_str} 근처에 주가 데이터가 없습니다.")
                                continue
                    except Exception as e:
                        print(f"의견 날짜 {opinion_date_str} 처리 중 오류: {e}")
                        continue
                ...
                scatter = ax.scatter(actual_date, close_price,
        ...
        # 7. 그래프 설정
        ax.set_title(f'{symbol} 주가 및 투자 의견', fontsize=16, fontweight='bold')
        ...
        print(f"{symbol} 그래프가 성공적으로 생성되었습니다.")
...
def thinking_opinion(title: str):
    opinion_up = ["상향", "매수", "투자 의견", "투자의견", "Buy", "목표가 상향", "중립→매수", "매도→중립", "평가 상향"]
    opinion_down = ["하향", "매도", "Sell", "목표가 하향", "중립→매도", "매수->중립", "평가 하향"]
...
def get_bank_from_title(title: str):
    result = ""
    banks = [
        Bank(name="테러다인", nickNames=["테러다인"]),
        Bank(name="셀레스티카", nickNames=["셀레스티카", "세레스티카"]),
    ...
    isFinish = False
    for bank in banks:
        bankNicks = bank.nickNames
        for nick in bankNicks:
            nickPattern = r'\b' + re.escape(nick) + r'\b'
            if re.search(nickPattern, title):
                result = bank
                isFinish = True
                break
        if isFinish:
    ...
    return result
def get_ticker_news(symbol: str, banks: [str]):
    try:
        # 1. save_db_001.xlsx 파일 읽기
        print("save_db_001.xlsx 파일을 읽는 중...")
        df = pd.read_excel("save_db_001.xlsx")
        ...
        print(f"전체 투자의견 뉴스 수: {len(df)}개")
        
        # 2. 2번째 열(keyword)에서 키워드 필터링
        title_column = df.columns[1]  # 2번째 열 (0-based index)
        ...
        for idx, row in df.iterrows():
            ...
            if re.search(pattern, title):
                if (len(banks) == 0):
                    ...
                    added = False
                    for bank in banks:
                        bankNicks = get_bank(bank).nickNames
                        for nick in bankNicks:
                            nickPattern = r'\b' + re.escape(nick) + r'\b'
                            if re.search(nickPattern, title):
                                filtered_rows.append(row)
        ...
        print(f"{symbol} 뉴스 수: {len(filtered_rows)}개")
...
def filter_opinion_news(symbol: str, banks: []):
    """투자 의견 관련 뉴스만 필터링하여 저장하는 함수"""
    print("=== 투자 의견 뉴스 필터링 시작 ===")

    # 필터링할 키워드 목록
    opinion_keywords = ["목표가 상향", "목표가 하향", "투자 의견", "투자의견", "목표가", "매수의견", "매도의견", "매수 의견", "매도 의견", "중립→매수", "매도→중립",
                        "중립→매도", "매수->중립", "Buy", "Sell", "평가 상향", "평가 하향"]


    try:
        # 1. save_db_001.xlsx 파일 읽기
        news = get_ticker_news(symbol, banks)
        if len(news) == 0:
            print("데이터가 없습니다.")
            return
